...
rule make_allele_mphf:
    ...
    output:
        number_of_alleles = output_folder + "/recall/allele_mphf/number_of_alleles.txt",
        alelle_mphf = output_folder + "/recall/allele_mphf/allele_mphf.pickle",
    threads: 1
    resources:
    ...
    run:
        from evaluate.deduplicate_pairwise_snps import AlleleMPHF
        allele_mphf = AlleleMPHF.build_from_list_of_snps_dfs_filepaths(input.snps_dfs)
        with open(output.number_of_alleles, "w") as number_of_alleles_fh, \
             open(output.alelle_mphf, "wb") as allele_mphf_fh:
...
rule make_pairwise_variants_mphf:
    ...
    output:
        number_of_pairwise_variants = output_folder + "/recall/pairwise_variants_mphf/number_of_pairwise_variants.txt",
        pairwise_variants_mphf = output_folder + "/recall/pairwise_variants_mphf/pairwise_variants_mphf.pickle",
        pairwise_variation_id_to_alleles_id = output_folder + "/recall/pairwise_variants_mphf/pairwise_variation_id_to_alleles_id.pickle",
    threads: 1
    resources:
        mem_mb = lambda wildcards, attempt: 16000 * attempt
    log:
    ...
    run:
        from evaluate.deduplicate_pairwise_snps import PairwiseVariationMPHF
        pairwise_variants_mphf = PairwiseVariationMPHF.build_from_list_of_snps_dfs_filepaths(input.snps_dfs, input.allele_mphf)
        with open(output.number_of_pairwise_variants, "w") as number_of_pairwise_variants_fh, \
             open(output.pairwise_variants_mphf, "wb") as pairwise_variants_mphf_fh, \
             open(output.pairwise_variation_id_to_alleles_id, "wb") as pairwise_variation_id_to_alleles_id_fh:
...
rule deduplicate_pairwise_snps:
    ...
    output:
        pangenome_variations_defined_by_allele_ids = output_folder + "/recall/deduplicate_pairwise_snps/pangenome_variations_defined_by_allele_ids.pickle",
    threads: 1
    resources:
        mem_mb = lambda wildcards, attempt: 16000 * attempt
    log:
...
rule convert_pangenome_variations_to_deduplicated_snps_df:
    input:
        ...
        snps_dfs_filepaths = expand(output_folder + "/recall/snps_dfs/{sample_pair_as_str}.snps_df.pickle", sample_pair_as_str=sample_pairs_as_str),
    output:
        deduplicated_snps_dfs_filepaths = expand(output_folder + "/recall/deduplicated_snps_dfs/{sample_pair_as_str}.snps_df.pickle", sample_pair_as_str=sample_pairs_as_str),
        deduplicated_snps_dfs_text_filepaths = expand(output_folder + "/recall/deduplicated_snps_dfs/{sample_pair_as_str}.snps_df.csv", sample_pair_as_str=sample_pairs_as_str)
    threads: 1
    resources:
        mem_mb = lambda wildcards, attempt: 16000 * attempt
    log:
    ...
    run:
        import pickle
        from evaluate.deduplicate_pairwise_snps import AlleleMPHF, PangenomeVariations, ConsistentPangenomeVariations

        with open(input.allele_mphf_filepath, "rb") as allele_mphf_file, \
             open(input.pangenome_variations_defined_by_allele_ids_filepath, "rb") as pangenome_variations_defined_by_allele_ids_file:
        ...
        for snps_df_filepath, deduplicated_snps_df_filepath, deduplicated_snps_df_text_filepath \
                in zip(input.snps_dfs_filepaths, output.deduplicated_snps_dfs_filepaths, output.deduplicated_snps_dfs_text_filepaths):
