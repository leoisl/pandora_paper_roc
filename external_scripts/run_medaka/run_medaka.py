...
def get_args():
    parser = argparse.ArgumentParser(description="Run medaka using Rachel's nextflow pipeline.")
...
def main():
    ...
    output_dir = output_dir / f"nanopore/{coverage}/random"
    output_dir.mkdir(parents=True, exist_ok=True)

    for sample_id, sample_path in zip(samples["sample_id"], samples["sample_path"]):
        print(f"Processing {sample_id}...")
        sample_path = Path(sample_path).absolute()
        medaka_output_dir = output_dir / sample_id
        medaka_output_dir.mkdir(parents=True, exist_ok=True)
        run_command(f"cd {medaka_output_dir} && "
                    f"bsub.py 2 run_genotype_medaka_{sample_id} "
                    f"nextflow run {script_dir / 'medaka.nf'} -w work -c {script_dir / 'nextflow.config'} "
